...
| Chat/API **504 Gateway Timeout** (~60s) | Host nginx/Caddy default `proxy_read_timeout` | Set `proxy_read_timeout 600s` on every reverse proxy to the app; see `platform/deploy/nginx-host-docker.conf.example` |
